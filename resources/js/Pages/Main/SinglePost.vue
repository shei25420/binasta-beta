                        ...
                        <aside class="sidebar-wrapper">
                            ...
                            <div class="single-sidebar">
                                ...
                                <div v-for="new_post in new_posts" :key="new_post.slug" class="media mt-5">
                                    <Link :href="'/blog/' + new_post.slug"><img class="mr-3"
                                        :src="'/storage/' + new_post.image_path" alt="" /></Link>
                                    <div class="media-body align-self-center">
                                        <h5>
                                            <Link :href="'/blog/' + new_post.slug">{{ new_post.post }}</Link>
                                        </h5>
                                        <span>
                                            <Link :href="'/blog/' + new_post.slug">{{
                                                new
                                                                                        Date(new_post.created_at).toDateString()
                                            }}</Link>
                                        </span>
                                    </div>
                                </div>
